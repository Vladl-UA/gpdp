...
// «Позже») — контекст определяется раньше snapshot_init(). Пока
// только whitelist + заглушка для не-data (стадии 0-4 сделаны, стадия
// 5 — условная загрузка отсюда — не начата).
...
@file_get_contents('http://localhost:8088/index.php?_context=configurator&_table=main&_action=view');
check('_context=configurator -> 501 (стадия 5 не начата, не тихий провал)',
    str_contains(implode(' ', $http_response_header ?? []), '501'));
