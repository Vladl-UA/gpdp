 ...
 *
 * ВНИМАНИЕ, не скрываем: последовательность IDENTITY не откатывается
 * транзакцией даже при ROLLBACK (общее свойство SQL-последовательностей,
 * не специфика диалекта) — dry-run не портит данные, но «сжигает»
 * несколько номеров id у следующей настоящей вставки. Не критично
 * функционально, но нечестно было бы молчать об этом. Формулировка
 * поправлена 2026-07-19: до этого здесь оставалось «auto_increment у
 * InnoDB» — след MySQL, переживший переезд 07-16.
 */
...
if ($dry_run) {
    // 2026-07-19: было сырое pg_query($db, 'BEGIN') мимо db.php и без
    // проверки результата — при неудачном BEGIN «пробный» прогон писал
    // бы в базу по-настоящему, молча. Теперь примитив свой (db.php) и
    // отказ виден сразу, до первой вставки.
    $begin = db_transaction_begin($db_connection);
    if (!$begin['ok']) {
        fwrite(STDERR, 'Не удалось открыть транзакцию для --dry-run: ' . $begin['error'] . "\n");
        fwrite(STDERR, "Прогон остановлен: без транзакции dry-run записал бы данные по-настоящему.\n");
...
if ($dry_run) {
    $rollback = db_transaction_rollback($db_connection);
    if (!$rollback['ok']) {
        fwrite(STDERR, "\nВНИМАНИЕ: откат не выполнен (" . $rollback['error'] . ") — данные могли остаться в базе.\n");
    } else {
